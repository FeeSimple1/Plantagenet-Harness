...
> in the Plantagenet-Harness repo as a catalog of **engineering patterns
> and audit techniques only**. It is NOT a source of Plantagenet rules.
>
> **Everything game-specific below is Nevsky, not Plantagenet, and is
> purely illustrative of the pattern being described.** That includes,
> without limitation: every `SMOKE-NNN` number; file paths like
> `src/nevsky/…`; module names (`legal_moves.py`, `campaign.py`,
> `events.py`); card identifiers (`R11`, `T18`, etc.); Locale/mechanic
> names (Veche, Storm/Sally, Druzhina, dorpat/odenpah, Teutonic/Russian,
> off-Calendar service, etc.); and the "200 rounds / 114 SMOKEs" history.
>
> **Do not encode any Nevsky rule, datum, card, Locale, or mechanic into
> this harness.** When applying a pattern, re-derive it from Plantagenet's
> own rules and reference files. Translate Nevsky terms to their
> Plantagenet analogues only as engineering guidance: e.g. the
> enumerator/handler split, the "define each predicate once" rule, the
> mirror-gap and lifecycle-leak audits, and the round-trip sweep all apply
> to any Levy & Command engine — the specific examples do not.
>
> ---
>
