> # ⚠️ PORTING NOTE — read before using this document
>
> This document was authored for the **Nevsky-Harness** and is included
> in the Plantagenet-Harness repo as a catalog of **bug-pattern audit
> techniques only**. It is NOT a source of Plantagenet rules.
>
> **Everything game-specific below is Nevsky, not Plantagenet, and is
> purely illustrative.** That includes every `SMOKE-NNN`; the
> per-pattern counts and the "114 Nevsky bugs" tally; file/module paths
> under `src/nevsky/`; card ids (`R4`, `T15`, `T17`…); and Nevsky
> mechanics and Locales (Veche, Castle/Walls overlays, Storm/Sally,
> parallel Trackway/Waterway at dorpat↔odenpah, off-edge Calendar
> service, Spoils, ransom, etc.). Plantagenet has its own forces
> (Longbow, Militia, Mercenaries, Handgunners…), its own map (Road/
> Highway/Path + Sea, Exile boxes), and its own cards.
>
> **Do not encode any Nevsky rule, datum, card, Locale, or mechanic into
> this harness.** Use the *patterns* (state-set-but-unreachable, mirror
> gaps, stale per-Lord flags, lifecycle leaks, rule-cite-but-no-enforce,
> no-target-no-op events, etc.) and re-derive every concrete check from
> Plantagenet's own Rules of Play and reference files. Where a pattern
> names a Nevsky concept, find the Plantagenet analogue before acting.
>
> ---
>
